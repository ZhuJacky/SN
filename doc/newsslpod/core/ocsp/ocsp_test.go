...
func TestOCSPDecode(t *testing.T) {
	...
	if resp.Status != expected.Status {
		t.Errorf("resp.Status: got %v, want %v", resp.Status, expected.Status)
	}
	...
	if resp.RevocationReason != expected.RevocationReason {
		t.Errorf("resp.RevocationReason: got %v, want %v", resp.RevocationReason, expected.RevocationReason)
	}
}
...
func TestOCSPResponse(t *testing.T) {
	...
	if resp.Status != template.Status {
		t.Errorf("resp.Status: got %v, want %v", resp.Status, template.Status)
	}
	...
	if resp.RevocationReason != template.RevocationReason {
		t.Errorf("resp.RevocationReason: got %v, want %v", resp.RevocationReason, template.RevocationReason)
	}
}
...
func TestErrorResponse(t *testing.T) {
	...
	if respErr.Status != Malformed {
		t.Fatalf("expected Malformed status from ParseResponse but got %v", respErr.Status)
	}
}
...
func TestOCSPRootCert(t *testing.T) {
	var block *pem.Block
	block, _ = pem.Decode([]byte(BAIDU))
	if block == nil {
		log.Warn("根证书解析失败")
	}
	cert, _ := x509.ParseCertificate(block.Bytes)
	ocspRaw := GetOCSPdata(context.Background(), cert, nil)
	if ocspRaw == nil {
		log.Warn("ocsp信息为空 ")
		return
	}
	rocsp, err := ParseResponse(*ocspRaw, nil)
	if err != nil {
		log.Warn("ocsp信息错误")
		return
	}
	log.Infof("rocsp.Status : %v", rocsp.Status)

}

var deepzz = `-----BEGIN CERTIFICATE-----
...
var revokebadssl = `-----BEGIN CERTIFICATE-----
...

...

-----END CERTIFICATE-----`

func TestCheckRevoke(t *testing.T) {
	assert := assert.New(t)
	var block *pem.Block
	//block, _ = pem.Decode([]byte(deepzz))
	//assert.NotNil(block, "deepzz 证书解析失败")
	//cert, _ := x509.ParseCertificate(block.Bytes)
	//ocspRaw := GetOCSPdata(context.Background(), cert, nil)
	//assert.NotNil(ocspRaw, "deepzz 证书解析错误")
	//rocsp, err := ParseResponse(*ocspRaw, nil)
	//assert.Nil(err, "ocsp 获取失败")
	//assert.Equal(Good, rocsp.Status, "deepzz 证书吊销")
	//
	//block, _ = pem.Decode([]byte(revokebadssl))
	//assert.NotNil(block, "revokebadssl 证书解析失败")
	//cert, _ = x509.ParseCertificate(block.Bytes)
	//ocspRaw = GetOCSPdata(context.Background(), cert, nil)
	//assert.NotNil(ocspRaw, "revokebadssl 证书解析错误")
	//rocsp, err = ParseResponse(*ocspRaw, nil)
	//assert.Nil(err, "ocsp 获取失败")
	//assert.Equal(Revoked, rocsp.Status, "revokebadssl 没有被吊销")
