...
var (
	ErrSeqInvalid      = errors.New("seq invalid")
	ErrConnPopTimeout  = errors.New("pop conn timeout")
	ErrConnPushTimeout = errors.New("push conn timeout")
)
...
func (c *Api) pushConn(conn net.Conn) error {
	...
	case c.conn <- conn:
		return nil
	case <-time.After(c.Timeout):
		return ErrConnPushTimeout
	}
}

func (c *Api) dial(cmd int32, args ...interface{}) ([]byte, error) {
	...
	for _, v := range args {
		...
		case net.IP:
			if len(v.(net.IP)) == 16 {
				binary.Write(buf, c.Endian, v.(net.IP)[12:16])
			} else {
				ipv4int := big.NewInt(0)
				ipv4int.SetBytes(v.(net.IP).To4())
				binary.Write(buf, c.Endian, int(ipv4int.Int64()))
			}
		default:
			if err := binary.Write(buf, c.Endian, v); err != nil {
				return nil, err
			}
		}
	}
	...
	//fmt.Printf("%d\n", c.seq)
	c.seq++
	for _, v := range []interface{}{cmd, uint32(20 + buf.Len()), c.seq, int32(0), uint32(0)} {
		if err := binary.Write(head, c.Endian, v); err != nil {
			return nil, err
		}
	}
	...
	seq := c.Endian.Uint32(result[8:12])
	if c.seq != seq {
		return nil, ErrSeqInvalid
	}
	code := c.Endian.Uint32(result[12:16])
	if code > 5 || code < 0 {
