...
func (s *Server) Report(args ...bool) error {
	...
	if s.domain != nil {
		if s.allocCount > 0 {
			_, err := s.domain.api.dial(QOS_CMD_GET_STAT, s.domain.Mod, s.domain.Cmd, s.Ip, uint32(s.Port), s.allocCount, uint32(os.Getpid()))
			if err != nil {
				return err
			}
		}
		if s.errCount > 0 {
			_, err := s.domain.api.dial(QOS_CMD_CALLER_UPDATE_BIT64, int32(0), int32(0), s.domain.Mod, s.domain.Cmd, s.Ip, uint32(s.Port), int32(-1), s.errCount, s.errDelay, int32(os.Getpid()))
			if err != nil {
				return err
			}
		}
		if s.succCount > 0 {
			_, err := s.domain.api.dial(QOS_CMD_CALLER_UPDATE_BIT64, int32(0), int32(0), s.domain.Mod, s.domain.Cmd, s.Ip, uint32(s.Port), int32(0), s.succCount, s.succDelay, int32(os.Getpid()))
			if err != nil {
				return err
			}
		}
